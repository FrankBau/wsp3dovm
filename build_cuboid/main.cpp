...
struct Mesh : GeometricHexahedralMeshV3i
{
	void write_poly(const std::string filename)
	{
		...
		file << "# poly file generated by build_cuboid\n" << endl;
	...
	VertexHandle add_vertex(Vec3i& point) {
		for (auto v : vertices() ) {
			if (point == vertex(v)) {
				return v;
			}
		}
		return OpenVolumeMesh::GeometricHexahedralMeshV3i::add_vertex(point);
	}

	// x increases left -> right
	// y increases top -> bottom
	// z increases into the screen (front --> rear)
	void add_cuboid(int x, int y, int z, int dx, int dy, int dz)
	{
		VertexHandle ltf = add_vertex(Vec3i(x, y, z));
		VertexHandle ltr = add_vertex(Vec3i(x, y, z + dz));
		VertexHandle lbf = add_vertex(Vec3i(x, y + dy, z));
		VertexHandle lbr = add_vertex(Vec3i(x, y + dy, z + dz));
		VertexHandle rtf = add_vertex(Vec3i(x + dx, y, z));
		VertexHandle rtr = add_vertex(Vec3i(x + dx, y, z + dz));
		VertexHandle rbf = add_vertex(Vec3i(x + dx, y + dy, z));
		VertexHandle rbr = add_vertex(Vec3i(x + dx, y + dy, z + dz));
...
int main()
{
	...
	mesh.add_cuboid(0, 0, 0, 1, 6, 144);
	mesh.add_cuboid(1, 0, 0, 1, 6, 144);
